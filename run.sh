...
cat data/azure.json | jq -r '.values[] | .properties.addressPrefixes[]' | sort -u > ranges-azure.txt
# for labels containing "DoD" only
cat data/azure.json | jq -r '.values[] | select(.name | test("dod"; "i")) | .properties.addressPrefixes[]' | sort -u > ranges-azure-dod.txt
